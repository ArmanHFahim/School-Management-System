...
class AdminController extends Controller
{
    ...
    public function paymentSettings()
    {
        ...
        $all_payments = $this->count_payment_gateway();
        ...
        $payment_gateways = PaymentMethods::where('school_id', auth()->user()->school_id)->get();
         ...
         } elseif (count($payment_gateways) != count($all_payments)) {

            foreach ($all_payments as $payments) {

                $available = 0;
                foreach ($payment_gateways as $single_gateway) {
                    if ($payments == $single_gateway->name) {
                        $available = 1;
                        break;
                    }
                }
    ...
    public function count_payment_gateway()
    {
        $all_payment_gateways = array(
            'paypal',
            'stripe',
            'razorpay',
            'flutterwave',
